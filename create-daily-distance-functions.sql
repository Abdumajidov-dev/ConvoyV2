-- Create/Update PostgreSQL functions for daily distance reports
-- These functions work with users.user_id (INTEGER) instead of users.id (BIGINT)

-- Function 1: Upsert daily distance report for single user
...
BEGIN
    ...
    -- Get all locations for this user on this date
    SELECT
...
-- Function 2: Generate daily reports for all users on a specific date
...
SELECT 'upsert_daily_distance_report function created successfully!' AS status;
SELECT 'generate_daily_reports_for_date function created successfully!' AS status;

-- Test: Show function signatures
SELECT
    routine_name,
    routine_type,
    data_type
...
WHERE routine_schema = 'public'
  AND routine_name IN ('upsert_daily_distance_report', 'generate_daily_reports_for_date')
